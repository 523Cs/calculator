...
let billAmount = document.getElementById("billAmount").value;      //sets a variable for the bill amount selector using the VALUE of the #billInput selector


/* ---------------------------------------Step 2: turn strings into numbers, use parseFloat() method to do this.-----------  IE: let vairable = parseFloat(stringVariable)----------------------------- */
  let tip = parseFloat("tipAmount"); // sets a variable for the Tip Amount that is a number
  let bill = parseFloat("billAmount");                        // sets a variable for the Bill Amount that is a number
...
/* --------------------------------Step 4: Place your calculations here----------------------------------------------- */
  ...
  let totalBill = ("tipAmount + billAmount");  // calcualte total bill (totalTip + billAmount) and store it in a new variable


/* --------------------------------Step 5: Append your HTML content with the new information-----**use .fixed(2) to display the value with 2 decimal places.-------------------- */
    tip.innerHtml = "$" + totalBill.toFixed(2)       // change the innerHTML of the tip var from step 3 to the total tip from step 4
    bill.innerHtml = "$" + billPrice.toFixed(2)            // change the innterHTML of the bill var from step 3 to the total bill var from step 2
